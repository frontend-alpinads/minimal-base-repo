        ...
        <div className="-ml-4 flex lg:-ml-5">
          {slides.map((slide, index) => (
            ...
            >
              ...
              <div
                className={cn(
                  "bg-foreground absolute inset-y-0 left-0 w-0 transition-all duration-300",
                  selectedIndex === index ? "w-5 max-lg:w-3" : "",
                )}
              />
              <div
                className={cn(
                  "bg-foreground absolute inset-x-0 top-0 h-0 transition-all duration-300 max-lg:hidden",
                  selectedIndex === index ? "" : "h-8",
                )}
              />
              <div
                className={cn(
                  "bg-foreground absolute inset-x-0 bottom-0 h-0 transition-all duration-300 max-lg:hidden",
                  selectedIndex === index ? "" : "h-8",
                )}
              />
            </div>
