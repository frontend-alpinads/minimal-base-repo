...
export function RoomsCarousel({
  ...
  return (
    <div className="relative lg:pl-10">
      <div className="overflow-hidden px-4 lg:px-5 lg:pl-0" ref={emblaRef}>
        <div className="-ml-4 flex lg:-ml-6">
          {slides.map((slide, index) => (
            <div
              key={index}
              className="flex-[0_0_100%] pl-4 md:flex-[0_0_50%] lg:flex-[0_0_auto] lg:pl-6"
            >
              {slide}
            </div>
          ))}
      ...
      {/* Navigation Buttons */}
      <div className="flex flex-col gap-8 px-4 pt-5 lg:flex-row lg:items-center lg:gap-10 lg:px-5 lg:pt-6 lg:pl-0">
        <div className="flex justify-between gap-4 lg:flex-none lg:shrink-0 lg:gap-10">
          <p className="text-background flex items-center justify-center opacity-80 lg:flex-none lg:text-base">
            {getViewingText()
              .replace("{current}", String(selectedIndex + 1))
              .replace("{total}", String(slides.length))}
          </p>

          <div key={selectedIndex} className="flex gap-4 lg:order-last">
            <button
              onClick={scrollPrev}
              className="bg-background/0 hover:bg-background/3 border-background/30 flex size-12 cursor-pointer items-center justify-center rounded-full border transition-all disabled:pointer-events-none disabled:opacity-50"
        ...
        <div
          key={selectedIndex}
          className="bg-background/50 h-1 max-lg:hidden lg:order-first lg:flex-1"
        >
          <div
