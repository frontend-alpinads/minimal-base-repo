...
export function FeaturesCarousel({ slides }: OffersCarouselProps) {
  ...
  return (
    <div className="relative">
      <div className="overflow-hidden px-4 lg:px-5" ref={emblaRef}>
        ...
        >
          {slides.map((slide, index) => (
            <div
              key={index}
              className="embla__slide flex-[0_0_100%] pl-0 transition-transform duration-300 md:flex-[0_0_50%] lg:flex-[0_0_auto] lg:pl-6 [&:not(.is-snapped)]:scale-[0.9] md:[&:not(.is-snapped)]:scale-100 lg:[&:not(.is-snapped)]:scale-100"
            >
              {slide}
            </div>
          ))}
      ...
      {canScroll && (
        <div className="flex justify-center gap-4 px-5 pt-5 lg:justify-end lg:pt-10">
          <button
            onClick={scrollPrev}
