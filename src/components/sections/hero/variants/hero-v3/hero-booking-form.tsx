...
export function HeroBookingForm({
  ...
  return (
    <div
      className={
        "lg:bg-card grid w-full grid-cols-[2fr_2fr_1fr] gap-6 overflow-hidden max-lg:flex max-lg:flex-col lg:items-center lg:gap-8 lg:p-3"
      }
    >
      {/* Date Selection */}
      {isDesktop ? (
        ...
        >
          <button className="group flex flex-1 cursor-pointer items-center gap-4 text-start">
            <div className="bg-background/20 group-hover:bg-primary/10 border-foreground/10 lg:bg-background/30 lg:group-hover:bg-background/50 flex size-14 items-center justify-center border transition-all duration-300">
              <CalendarDotsIcon className="text-foreground lg:text-background size-6" />
            </div>
            <span className="flex flex-1 flex-col gap-2">
              <span className="text-foreground/70 lg:text-background/70 text-sm leading-none font-normal">
                {hero.bookingForm.arrivalDepartureLabel}
              </span>
              <span className="text-foreground lg:text-background text-base leading-none">
                {formatDateDisplay()}
              </span>
            </span>

            <CaretDownIcon className="text-foreground lg:text-background size-6" />
          </button>
        </DesktopDatePicker>
      ) : (
        <MobileDatePicker
      ...
      >
        <button
          className={cn(
            "group hidden flex-1 cursor-pointer items-center gap-4 text-start lg:flex",
            hideGuestSelectionInMobile && "hidden lg:flex",
          )}
        >
          <div className="bg-background/20 group-hover:bg-primary/10 border-foreground/10 lg:bg-background/30 lg:group-hover:bg-background/50 flex size-14 items-center justify-center border transition-all duration-300">
            <UsersIcon className="text-foreground lg:text-background size-6" />{" "}
          </div>
          {/* Guests Selection */}
      ...
      <Button
        variant={isDesktop ? "white" : "default"}
